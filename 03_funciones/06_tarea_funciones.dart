...
void impCmd(dynamic mensaje) => stdout.writeln(mensaje);

String impReadCmd(String mensaje){
  impCmd(mensaje);
  return stdin.readLineSync();
}

void calcularDeducciones(double salario, Map<String, dynamic> usuario){
  usuario['salario']     = salario;
  usuario['deducciones'] = salario * 0.15;
  usuario['salarioNeto'] = salario - (salario*0.15);
  impCmd(usuario);
}

void procesarUsuario(double salario){
  final Map<String, dynamic> usuario = {};
  usuario['nombre'] = impReadCmd('¿Cúal es su nombre?');
  usuario['edad'] = impReadCmd('¿Qué edad tienes?');
  usuario['pais'] = impReadCmd('¿En qué país naciste?');

  impCmd('Usuario 1 sin deducciones');
  impCmd(usuario);

  calcularDeducciones(salario, usuario);
}
